...
INCORPORATE BANK CLASS FUNCTION AND HELPER FUNCTIONS

_replaceNodesWithBankObjects replaces the nodes in a given networkx network 
generated by whatever algorithm with Bank objects that have added 
...
def _replaceNodesWithBankObjects(G, Tl, Ts):
    ...
    banks = createBanks(G.number_of_nodes())
    # Link the new Bank objects like how the old nodes were linked
    return linkBanks(G, banks)

''' Create a given number of Bank objects '''
...
MEAN FIELD NETWORK HELPER FUNCTIONS
