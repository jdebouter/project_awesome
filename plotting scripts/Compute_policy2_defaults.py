...
POLICY_sd = []
for P in POLICY:
    parameters['too_big_to_fail'] = P
    parameters['too_big_to_spread'] = P
    for i in range(100):
    ...
    total_default_list = [sum(lst)/100.0 for lst in avalanche_sizes]
    POLICY_mean.append(np.mean(total_default_list))
    POLICY_sd.append(np.std(total_default_list))

plt.errorbar([2, 2], POLICY_mean, POLICY_sd)
plt.show()
